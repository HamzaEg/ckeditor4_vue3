<template>
    <div>
        ...
        <v-row align="center" class="d-flex justify-center">
          <v-col class="d-flex" cols="12" sm="6">
            ...
            <v-select
              ...
              persistent-hint
              return-object
              @update:modelValue="setEditorData(selectedPriority)"
            ></v-select>
          </v-col>
          <!-- @click="setEditorData" -->
        </v-row>
      ...
      <v-card color="grey pa-3 mb-3">
        <textarea id="brief" name=""></textarea>
      </v-card>
  ...
  export default {
    ...
    setup(props, { emit }) {
      ...
      onMounted(() => {
        // initalize ckeditor
        ckEditor = window.CKEDITOR.replace("brief");
        ckEditor.setData("");
    ...
    data() {
      return {
        h3s: ["Diagnosen", "Anamnese", "Epikrise"],
        templates: [
